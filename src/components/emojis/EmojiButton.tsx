import { CSSProperties, ReactNode, useCallback, useState, useEffect, MouseEvent } from 'react';
...
export const EmojiButton = (props: EmojiButtonProps) => {
  const { id, className, style, children, onClick, animated, emoji, native } = props;
  ...
  return (
    ...
    >
      ...
      <div className='ne-emoji-button-icon'>{native ? emojiToUse : <Twemoji>{emojiToUse}</Twemoji>}</div>
    </li>
  );
};
