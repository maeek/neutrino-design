...
export const SliderInput = (props: SliderInputProps) => {
  ...
  const [dimensions, setDimensions] = useState<BarDimensions | null>(null);
  const [value, setValue] = useState(initValue);
  const stepsLength = (steps?.length || 0) - 1;
  ...
  const stepper = useCallback(
    (pos: number, dms: number) => {
      if (!steps || !dimensions) {
        return pos;
      }

      const stepsValues = steps
        .map(({ value: innerValue }) => innerValue)
        .map((v, i) => ((v ?? Math.round(100 - (100 - i * (100 / stepsLength)))) * Math.round(dms)) / 100);

      return stepsValues.reduce((prev, curr) => (Math.abs(curr - pos) < Math.abs(prev - pos) ? curr : prev));
    },
  ...
  const moveThumb = useCallback(
    (axisPos: number) => {
      ...
      if (axis === SliderTypes.HORIZONTAL) {
        ...
        if (axisPos < dimensions.bar.left + dimensions.bar.width && axisPos > dimensions.bar.left) {
        ...
        if (axisPos < dimensions.bar.top + dimensions.bar.height && axisPos > dimensions.bar.top) {
          if (pos <= 0) {
            restrictToBoundry = 0;
        ...
        setPosition(restrictToBoundry ?? 0);
        setValue(Math.round(((restrictToBoundry ?? 0) / (dimensions.bar.height - dimensions.thumb.height)) * 100));
      }
    },
  ...
  const handleDown: MouseEventHandler<HTMLDivElement> & TouchEventHandler<HTMLDivElement> = e => {
    ...
    if (e.clientX || e.clientY) {
      moveThumb(axis === SliderTypes.HORIZONTAL ? e.clientX : e.clientY);
    } else {
      moveThumb(axis === SliderTypes.HORIZONTAL ? e.targetTouches[0].clientX : e.targetTouches[0].clientY);
    }
  };
  ...
  const defaultTooltip = <div className='ne-input-slider-tooltip-default'>{Math.round(value)}%</div>;
  ...
  const stepsNode = steps?.map(({ label, value }, i) => {
    ...
    return (
      ...
      >
        {label && (
          <div
            className={`ne-input-slider-bar-steps-step-tooltip ne-input-slider-bar-steps-step-tooltip--${tooltipPos}`}
          >
            {typeof label === 'function' ? label(percent) : label}
          </div>
        )}
      </div>
    );
  ...
  const tooltipFromSteps = steps && stepsTooltipProgress && (
    <div className='ne-input-slider-tooltip-steps'>
      {typeof stepsTooltipProgress === 'function' ? stepsTooltipProgress(value) : stepsTooltipProgress}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className='ne-input-slider-bar-steps'
          draggable={false}
        >
          {stepsNode}
        </div>
        ...
        >
          <div className={`ne-input-slider-tooltip ne-input-slider-tooltip--${tooltipPos}`}>
            {typeof tooltip === 'function' ? tooltip(Math.round(value)) : tooltip || tooltipFromSteps || defaultTooltip}
          </div>
        </div>
      </div>
    </div>
