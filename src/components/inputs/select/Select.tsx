...
export const Select = (props: SelectProps) => {
  const { items, onChange, selectedIndexes, className, placeholder, multiple, highlightSearched } = props;
  const { onEnterOrSpace, onArrowUp, onArrowDown, onEscape } = useAccessibility();
  ...
  const handleChange = (item: ItemDefinition) => {
    if (multiple) {
      setSelectItems(p => (p?.includes(item.index) ? p?.filter(i => i !== item.index) : [...(p || []), item.index]));
    } else {
      setSelectItems(p => (p !== null && p?.includes(item.index) ? null : [item.index]));
    }
  };

  const onItemKeyUpHandler = (item: ItemDefinition, i: number) => (e: KeyboardEvent<Element>) => {
  ...
  useEffect(() => {
    ...
    if (isOpened) {
      ...
      itemsRef.current[firstIndexFound]?.current?.focus();
      return;
    }
      ...
      {items?.map((item, i) => {
        ...
        return (
          <li
            ...
            tabIndex={0}
            role='option'
            aria-selected={selectedItems?.includes(item.index)}
  ...
  const isPlaceholder =
    (!selectedItems || (Array.isArray(selectedItems) && selectedItems.length === 0)) && !searchValue;
  const activeClasses = {
          ...
          {searchValue ||
            (Array.isArray(selectedItems) && selectedItems.length > 0
              ? items
                  ...
                  .join(', ')
              : placeholder)}
          <div className={classNames('ne-select-active-icon', { 'ne-select-active-icon--opened': isOpened })}>
            <KeyboardArrowDown />
          </div>
        </div>
        {isOpened && listNode}
